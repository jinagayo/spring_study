...
public class UserController {
	...
	/*
	 * AOP 설정필요 : UserLoginAspect 클래스에 userIdCheck 메서드로 구현
	 *  1. 로그인여부 검증
	 *     로그아웃 상태인 경우 로그후 거래 메세지 출력. login 페이지로 이동
	 *  2. 본인 거래 여부 검증
	...
	public ModelAndView idCheckDelete(String password, String userid,
		...
		if(userid.equals("admin")) {
			throw new ShopException("관리자는 탈퇴 불가", "mypage?userid=" + userid);
		}
		// 비밀번호 검증 : 로그인된 정보
		User loginUser = (User)session.getAttribute("loginUser");
		// 비밀번호 불일치
	 ...
	 */
	@GetMapping("password")
	public String loginCheckForm(String password, String chgpass,
			HttpSession session) {
		return null;
	}
	...
	public String loginCheckPassword(String password, String chgpass,
		...
		if(!password.equals(loginUser.getPassword())){
			throw new ShopException("비밀번호를 확인하세요.", "password?userid=" + loginUser.getUserid());
		}
		// 비밀번호 일치
				try {
					//db의 비밀번호 변경
					...
					e.printStackTrace();
					throw new ShopException("비밀번호 수정시 오류발생.", "password?userid=" +loginUser.getUserid());
				}
 ...
 */
	...
	mav.setViewName("search");  // 뷰만 서버에서 바뀐 것이고, 클라이언트의 주소창에는 변화가 없음 (서버 포워드)
	return mav;
}
}
